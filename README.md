<img src="src/icon.png" alt="찜꽁 Helper 아이콘" width="120" />

## 🌟 About This Project
This repository is a community-driven utility tool built for the **Woowa Tech Course**, an elite software engineering program operated by South Korea's leading tech conglomerate. 

While it may not have thousands of stars, hundreds of active developers and applicants in our local ecosystem **quietly depend on this tool every day** to streamline their development workflows, save time, and improve team collaboration.

# 📌 찜꽁 Helper (Chrome Extension)


찜꽁 사이트에서 회의실/페어링 존 예약을 편하게 처리할 수 있도록 돕는 Chrome 확장 프로그램입니다. ✨
- 제작: `8기 프론트엔드 파라디`


---
## 🆕 0.1.0 대비 1.1.0 변경점

- 회의실 전용 타임테이블에서 **회의실/페어링 존 탭형 구조**로 확장
- 층/공간 매핑 고도화 (회의실 + 페어링 존 층별 구분)
- 시간 선택 길이 옵션 추가: **10~60분** (기본 1시간 자동 선택)
- 확장 팝업에 **내 예약 조회/예약별 공유** 기능 추가
- 안내 UX를 alert 중심에서 **토스트 중심**으로 정리하고, 야간 표시/노출 안정화

---

## 🧭 1. 프로그램 소개

찜꽁 Helper는 우테코 판교 캠퍼스 공간(회의실/페어링 존) 예약 과정에서 반복되는 조작을 줄이고, 시간/공간 선택을 빠르게 완료할 수 있도록 설계되었습니다.

핵심 목적
- 🔎 예약 가능 시간대를 한눈에 확인
- 🖱️ 시간 블록 클릭만으로 예약 폼 자동 입력
- ✍️ 사용 목적 입력까지 자연스럽게 이어지는 UX 제공

---

## ✨ 2. 주요 기능

### 2-1. 메인 기능: 날짜/시간/공간 자동 선택
- 🏢 층/공간 기준 타임테이블 표시 (회의실/페어링 존 구분 탭 지원)
- ⏱️ 시간 선택 길이 버튼 제공: 10/20/30/40/50/60분 (기본 1시간)
- 🟩 빈 시간 클릭 시 선택된 길이만큼 자동 선택
- 📝 날짜/시작/종료/공간 폼 자동 반영
- 🔎 사용 목적 입력란으로 자동 스크롤/포커스

### 2-2. 확장 팝업: 내 예약 현황
- 👤 로그인 상태에서 내 예약 목록 조회
- 📋 각 예약 카드에 공간, 층, 시간, 사용 목적, 예약자 표시
- 🔗 각 예약별 `이 예약 공유` 버튼을 제공하여 공유 텍스트를 클립보드로 복사
```text
📌 찜꽁 예약 현황

- 시간: 2026-03-03 (화) 10:00 ~ 10:30
- 공간: 아폴로 (12층)
- 사용 목적: 연극 연습
- 예약자: 파라디
```

---

## 🛠️ 3. 사용 방법

### 3-1. 코드 받기

방법 1: Chrome 웹스토어에서 설치 (권장)

1. Chrome 웹스토어 열기: chrome.google.com/webstore
2. 검색창에 `찜꽁 Helper` 입력하여 확장 프로그램을 찾습니다.
3. `Chrome에 추가` 버튼을 클릭해 설치합니다.

방법 2: Github에서 직접 받기

방법 2-A: Git Clone

```bash
git clone https://github.com/JetProc/zzimkkong-helper.git
```

방법 2-B: Code에서 ZIP 다운

1. GitHub 저장소에서 `Code` → `Download ZIP` 클릭
2. 압축 해제


방법 2 사용 시 — Chrome 확장 등록

1. 주소창에 `chrome://extensions` 입력
2. 우측 상단 `개발자 모드` 활성화
3. `압축해제된 확장 프로그램을 로드합니다` 클릭
4. `manifest.json`이 있는 프로젝트 루트 선택

![확장 등록 화면](docs/images/install-extensions-page.png)

---

## 📌 4. 확장 프로그램 고정(핀) 방법

1. Chrome 우측 상단 퍼즐 아이콘(확장 프로그램) 클릭
2. 목록에서 `찜꽁 Helper` 찾기
3. `핀` 아이콘을 클릭해 고정
4. 툴바에 고정된 `찜꽁 Helper` 아이콘 클릭 시 팝업 실행

![핀 고정 화면](docs/images/pin-extensions-page.png)

---

## 🙋‍♂️ 5. 사용 방법

### 5-1. 찜꽁 Helper 사용
1. 찜꽁 사이트 접속
2. 날짜 선택
3. 구분 탭에서 회의실/페어링 존 선택
4. 시간 선택 길이 버튼 선택 (10/20/30/40/50/60분, 기본 1시간)
5. 타임테이블에서 상태 확인
- 🍂 주황색: 내 예약
- ☘️ 초록색: 비어 있음
- 🌹 빨간색: 예약 있음
- 🪨 회색: 지난 시간
6. 빈 블록 클릭 후 예약 폼 자동 반영 확인
7. 사용 목적 입력 후 예약 진행

### 5-2. 찜꽁 Helper 팝업 사용
1. 고정된 `찜꽁 Helper` 아이콘 클릭
2. 내 예약 목록 확인
3. 공유할 예약 카드에서 `이 예약 공유` 클릭
4. 복사된 텍스트를 슬랙 등에 붙여넣기

---

## 🏢 6. 층/공간 구성

- 회의실
  - 11층: 금성, 지구, 수성, 화성
  - 12층: 보이저, 디스커버리, 아폴로, 허블
  - 13층: 은하수
- 페어링 존
  - 12층: 페 14 ~ 페 7
  - 13층: 페 6 ~ 페 1

---

## ⚠️ 7. 트러블슈팅

### Q1. 확장이 로드되지 않아요
- `manifest.json`이 있는 폴더를 선택했는지 확인
- `chrome://extensions`의 오류 메시지 확인
- 확장 새로고침 후 페이지도 새로고침

### Q2. 팝업에서 예약이 안 불러와져요
- 찜꽁 게스트 페이지가 열려 있는지 확인
- 로그인 상태 확인
- 게스트 페이지와 확장을 함께 새로고침

### Q3. 공유 버튼 복사가 안돼요
- 브라우저 권한/보안 정책에 따라 클립보드가 막힐 수 있음
- 게스트 페이지가 열려 있는 상태에서 다시 시도
- 브라우저 권한(클립보드) 허용 후 재시도

### Q4. 내 예약(주황색) 표시가 안 보여요
- 팝업에서 내 예약이 정상 조회되는지 먼저 확인
- 게스트 페이지와 확장을 새로고침 후 다시 확인
- 선택한 날짜에 내 예약이 실제로 존재하는지 확인

---

## 📁 8. 프로젝트 구조

```text
zzimkkong-helper/
├─ manifest.json
├─ README.md
├─ docs/
│  └─ images/
└─ src/
   ├─ background.js
   ├─ content.js
   ├─ page-bridge.js
   ├─ popup.html
   ├─ popup.css
   ├─ popup.js
   ├─ icon.png
   ├─ fonts/
   │  └─ BMDOHYEON_ttf.ttf
   └─ vendor/
      └─ toastify-js/
         ├─ toastify.min.js
         └─ toastify.min.css
```

---

## 🎨 9. 제작

made by `8기 프론트엔드 파라디`

(() => {
  ...
  const EXTENSION_FONT_PATH = 'src/assets/fonts/BMDOHYEON_ttf.ttf';
    ...
    shadow.innerHTML = `
      <style>
        @font-face {
          ...
          src: url("${getExtensionFontUrl()}") format("truetype");
          font-display: swap;
        }
      </style>
  ...
  function renderMapCalendarOverlay(scheduleData) {
    ...
    const durationControls = createMapCalendarDurationControls((nextDurationMinutes) => {
      ...
      renderMapCalendarOverlay(nextScheduleData);
      showHelperToast(
        `자동 선택 시간이 ${getAutoPickDurationLabel(nextDurationMinutes)}으로 변경되었습니다.`,
        'info',
        1800
      );
    });
  ...
  function scrollMapCalendarBodyToCurrentTime(scrollContainer, currentRowElement) {
    ...
    const preferredVisualTop = Math.max(
      scrollContainer.clientHeight * CURRENT_TIME_TARGET_VIEWPORT_RATIO,
      minimumVisualTop,
      56
    );
  ...
  function buildFloorGroups(rooms) {
    ...
    rooms.forEach((room) => {
      ...
      if (!activeGroup || activeGroup.floorCategory !== floorCategory || activeGroup.spaceCategory !== spaceCategory) {
        activeGroup = {
          floorCategory,
          roomTypes: [],
          rooms: [],
  ...
  function shouldRestoreForcedHiddenHostTimePanels(target) {
    ...
    return Boolean(
      target.closest(
        "button[name='start'], button[name='end'], input[type='radio'][name='midday'], input[type='radio'][name='hour'], input[type='radio'][name='minute']"
      )
    );
  }
  ...
  function getExtensionIconUrl() {
    try {
      if (chrome?.runtime?.getURL) {
        return chrome.runtime.getURL('src/assets/images/icon.png');
      }
    } catch {
